...
#     @window_color and @window_color_active so both live in the same
#     palette. Bright = factor 1.0, dim = factor 0.5.
#   - If @window_color is unset: unset @window_color_active too.
...
while IFS=$'\t' read -r wid color; do
  if [[ -n "$color" ]]; then
    ...
    tmux setw -t "$wid" -u @window_color_active 2>/dev/null
  fi
