...
set-hook -g 'client-attached[1212]' 'run-shell -b "if [ \"$(uname)\" = Darwin ]; then /opt/homebrew/bin/python3.14 $HOME/agents-status/statusbar/run.py; elif [ -n \"$MSYSTEM\" ]; then :; else $HOME/.config/hypr/UserScripts/RenameWorkspaces.py; fi"'
set-hook -g 'client-detached[1212]' 'run-shell -b "if [ \"$(uname)\" = Darwin ]; then /opt/homebrew/bin/python3.14 $HOME/agents-status/statusbar/run.py; elif [ -n \"$MSYSTEM\" ]; then :; else $HOME/.config/hypr/UserScripts/RenameWorkspaces.py; fi"'
